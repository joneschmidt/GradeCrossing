...
#define LoopMS 34
// Occupied threshold - 3 means that reading must be in the high/low 3rd of the high/low range
...
//
  ...
  // do the integration over LoopMS time
    ...
    if (Tint > XingDat[Xidx].Largest[Sidx]*.15){// check range is meaningful
      Tint /= OccLimit;
      if (Vals[Sidx] <= XingDat[Xidx].Largest[Sidx]-Tint) // if in the lower limit
        {Occpied[Sidx] = true;
        }// in lower limit 
    } // if Tint
    } // if active sensor
