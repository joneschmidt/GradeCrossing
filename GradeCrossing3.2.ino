...
// *************************************************************************
// Revision history:
#define Version "GradeCrossing 3.2 2024/05/06 12:25"
...
typedef struct {
  ...
  int SnsrType; 
  // sensors (LeftSnsr[NumTrks], RightSnsr[NumTrks], XingSnsr[NumTrks])
...
//  SnsrType, LeftSnsr[NumTrks], RightSnsr[NumTrks], XingSnsr[NumTrks], 
  {  AlgType, A0,            A2,            A1,               
...
boolean       HoldTilClear [NumXings]; // dont do anything until all clear
...
// set center occupied flag & clear timeout
  CntrOcc = false;
...
// check endpoints changed
  for (Sidx = 0; Sidx < Cntr; ++Sidx){
    ...
    if (XingDat[Xidx].NowOccStat[Sidx] == StatClr && XingDat[Xidx].LastOccStat[Sidx] != StatExit){ 
     NewStat = StatClr;
	} // new stat clear

	// process newly StatOcc
    if (XingDat[Xidx].NowOccStat[Sidx] == StatOcc){
      if (XingDat[Xidx].LastOccStat[Sidx] == StatClr){
	...
	if (NewStat != -1) 
	  {
		#if Trace > 0
	    RptSnsr(Sidx);
		Serial.println  (" Changing to " + SenStat[NewStat]);
		#endif
   ...
   if (XingDat[Xidx].NowOccStat[Sidx] != XingDat[Xidx].LastOccStat[Sidx]){
    ...
    switch (XingDat[Xidx].LastOccStat[Sidx]){
	  case StatClr:
		...
		NewStat = StatOcc;
		TimeoutTM[Xidx] = 0;
  //  Serial.println ("Strt " + String(MaxAlertSecs * 1000.)); // jjj
  //  Serial.println ("Strt " + String((MaxAlertTM[Xidx] > millis())?MaxAlertTM[Xidx] - millis():0)); // jjj
       break;
	  default:
	    break;
	} // switch
...
// vvv if everything is clear, clear all last
  SnsrOcc[Xidx] = false;
  for (Sidx = Left; Sidx < Cntr + NumTrks; ++Sidx){
		#if Trace > 1
	    RptSnsr(Sidx);
		Serial.println  ();
		#endif
  ...
  if (SnsrOcc[Xidx] == false){
	#if Trace > 1
    Serial.println  ("All clear");
	#endif
	 HoldTilClear[Xidx] = false;
	 MaxAlertTM[Xidx] = 0;
  	 TimeoutTM[Xidx] = 0;
   for (Sidx = Left; Sidx < Cntr + NumTrks; ++Sidx)
     {XingDat[Xidx].LastOccStat[Sidx] = StatClr;}
   if (NxtAction == -1 && XingPhase[Xidx] == XingBusy) {
	   NxtAction = XingRaiseGate;
       NxtPhaseTM[Xidx] = millis() + GateDlyMs;
   } // if Busy do cleanup
   }
// ^^^ if everything is clear, clear all last
