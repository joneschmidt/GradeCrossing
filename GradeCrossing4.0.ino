...
// *************************************************************************
// Revision history:
#define Version "GradeCrossing 4.0 2025/12/09"
//   2025/12/08 J. Schmidt 4.0 Remove EEPROM stuff, NumGates is 0 or 1
//   2025/12/04 J. Schmidt 3.6 make EEPROM library dependent on NumGates
//   2025/11/08 J. Schmidt 3.5 allow 2nd set of center sensors
...
#define GateUp   HIGH // LOW// ---OccStat values
...
// *************************************************************************
// /* 
// DEFINE THE ACTIVE CROSSINGS - BASIC
...
typedef struct {
  // input sensors digital (DigType) or analog (AlgType)
  int SnsrType; 
  // sensors (LeftSnsr[NumTrks], RightSnsr[NumTrks], CntrSnsr[NumTrks*NumCntr])
...
// ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
// updated 25/12/09
// 1 left sensor, 1 right sensor, 1 crossing sensor, no gates
// DEFINES NumXings 1, NumTrks 1, NumCntr 1, NumGates 0 above to match
...
//  PORTS: LeftSnsr[NumTrks], RightSnsr[NumTrks], CntrSnsr[NumCntr * NumTrks],  
           A4,                A3,                 A2, 
// Delay seconds: WaitABitSecs array 
...
// ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
// checked & updated 12/7/25 new bootloader
// 3 trk south, 2 trks at crossing, 2 trk north, no gates
// Define the active crossings - NumXings crossings - NICASIO NORTHERN
#define NumXings 1 // Number of grade crossings
#define NumTrks  3 // each crossing - up to NumTrks tracks, up to 3 (4) sensors per track
#define NumCntr  1 // number of center sensors per track
#define NumGates 0 // number of gate controls per crossing
...
#define NumXings 1 // Number of grade crossings
#define NumTrks  2 // each crossing - up to NumTrks tracks, up to 3 (4) sensors per track
#define NumCntr  1 // number of center sensors per track
#define NumGates 0 // number of gate controls per crossing
// Define the crossing by the following array
XingDef Crossing [] =
...
// ============= LowerGate ======================
  #if NumGates > 0
  if (Crossing[Xidx].GatePort > 0){
  #if Trace > 1
    Serial.print  ("LowerGate " + String(Crossing[Xidx].GatePort));
       digitalWrite(Crossing[Xidx].GatePort, GateDown);
         Serial.println(" value " + GateDown);
  #endif
  } // if
  #endif
...
#if NumGates > 0
  if (Crossing[Xidx].GatePort != 0){
      digitalWrite(Crossing[Xidx].GatePort, GateUp);
      #if Trace > 1
         Serial.print  ("RaiseGate " + String(Crossing[Xidx].GatePort));
         Serial.println(" value " + GateUp);
      #endif
  } // if
  #endif
...
//  SnsrType, SnsrLst[NumSnsrs], FirstLite, SoundCntl, GatePort
      if (Crossing[Xidx].FirstLite > 0){
  ...
  if (Crossing[Xidx].GatePort != 0){
      pinMode(     Crossing[Xidx].GatePort,   OUTPUT);
      digitalWrite(Crossing[Xidx].GatePort,   GateUp);
  } // if
  //
